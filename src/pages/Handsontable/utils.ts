...
export function transformTreeData1(tree: any): any {
  ...
  tree.forEach((level1Node: any) => {
    ...
    const level1Rowspan = level1Node.children && level1Node.children.length > 0 ? 1 : 3;
    level1.push({ label: level1Node.title, colspan: level1Colspan, rowspan: level1Rowspan });
...
export function transformTreeKeys(tree: TreeNode[]): TreeNode[] {
  return tree.map(node => {
    ...
    if (newNode.children) {
      newNode.__children = transformTreeKeys(newNode.children);
